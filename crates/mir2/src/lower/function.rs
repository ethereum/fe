...
use crate::{
    ir::{
        body_builder::BodyBuilder, inst::InstKind, value::Local, BasicBlockId, FunctionBody,
        FunctionId, FunctionParam, InstId, Value, ValueId,
    },
    MirDb,
};
...
impl<'db, 'a> BodyLowerHelper<'db, 'a> {
    pub(super) fn lower_stmt(&mut self, stmt: &hir_def::Stmt) {
        // match &stmt.kind {
        //     hir_def::Stmt::Return(value) => {
        ...
        //     hir_def::Stmt::VarDecl { target, value, .. } => {
        //         self.lower_var_decl(target, value.as_ref(), stmt.into());
        //     }

        //     hir_def::Stmt::ConstantDecl { name, value, .. } => {
        ...
        //     hir_def::Stmt::For(target, iter, body) => self.lower_for_loop(target, iter, body),

        //     hir_def::Stmt::While(test, body) => {
        //         let header_bb = self.builder.make_block();
        //         let exit_bb = self.builder.make_block();

        //         let cond = self.lower_expr_to_value(test);
        //         self.builder.branch(cond, header_bb, exit_bb());
        ...
        //             let inc = self.builder.add(loop_idx, imm_one());
    ...
    pub(super) fn lower_var_decl(&mut self, var: &hir_def::PatId, init: Option<&hir_def::Expr>) {
        // match &var.kind {
    ...
    pub(super) fn declare_var(&mut self, name: &SmolStr, ty: TypeId) -> ValueId {
    ...
    pub(super) fn lower_expr(&mut self, expr: &hir_def::Expr) -> (InstId, TypeId) {
        ...
        //         match op.kind {
        //             hir_def::UnOp::Invert => self.builder.inv(value, expr.into()),
        //             hir_def::UnOp::Not => self.builder.not(value, expr.into()),
        //             hir_def::UnOp::USub => self.builder.neg(value, expr.into()),
        //         }
        //     }
        ...
        //     hir_def::Expr::Call(func, generic_args, args) => {
    ...
    fn lower_for_loop(
        ...
        // self.builder.jump(preheader_bb());
        ...
        // let imm_zero = self.builder.bind(imm_zero());
        ...
        // self.branch_eq(loop_idx, maximum_iter_count, exit_bb, entry_bb);
        ...
        // let iter_elem = self.builder.aggregate_access(iter, vec![loop_idx]());
        // self.builder
    ...
    // fn lower_assignable_value(&mut self, expr: &hir_def::Expr) -> AssignableValue {
    //     match &expr.kind {
    ...
    // fn lower_function_id(&mut self, function: &hir_def::Func, args: &[Id<Value>]) -> FunctionId {
    ...
    fn lower_call(
        &mut self,
        func: &hir_def::Expr,
        _generic_args: &Option<Vec<hir_def::GenericArg>>,
        args: &[hir_def::CallArg],
        ty: TypeId,
    ) -> InstId {
