...
use fe_resolver::{
    graph::GraphResolver,
    ingot::{basic_ingot_graph_resolver, BasicIngotNodeHandler},
};
...
fn ingot_graph_resolution() {
    ...
    println!("{ingot_a_url:?}");
    let mut handler = BasicIngotNodeHandler::default();
    let ingot_graph = resolver.graph_resolve(&mut handler, &ingot_a_url).unwrap();
    snap_test!(
