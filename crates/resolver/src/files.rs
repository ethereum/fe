    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            FilesResolutionDiagnostic::RequiredDirectoryMissing(url, path) => {
                write!(f, "Missing required directory '{}' in ingot at {}", path, url)
            }
        }
    }
}
...
impl Resolver for FilesResolver {
        ...
        H: crate::ResolutionHandler<Self>,
    {
        ...
        if !ingot_path.exists() || !ingot_path.is_dir() {
            return Err(FilesResolutionError::DirectoryDoesNotExist(ingot_url.clone()));
        }
        ...
        for required_dir in &self.required_directories {
            ...
            if !required_dir_path.exists() || !required_dir_path.is_dir() {
                self.diagnostics.push(FilesResolutionDiagnostic::RequiredDirectoryMissing(
                    ingot_url.clone(),
        ...
        for required_file in &self.required_files {
            ...
            if !required_path.exists() {
                self.diagnostics.push(FilesResolutionDiagnostic::RequiredFileMissing(
                    ingot_url.clone(),
                ...
                match fs::read_to_string(&required_path) {
                    ...
                    Err(error) => {
                        ...
                        self.diagnostics.push(
                            FilesResolutionDiagnostic::FileIoError(required_path, error),
                        );
                    }
                }
            }
        ...
        for pattern in &self.file_patterns {
            ...
            let entries = glob(pattern_path.as_str()).map_err(FilesResolutionError::PatternError)?;
