...
fn main() {
    let f: Foo = Foo { x: 42 };
        ...
        assert!(
            !result.is_empty(),
            "Should find goto target for Foo type annotation"
        );

        for target in &result {
            if let Some(path) = target.pretty_path(&db) {
                assert!(
                    path.contains("Foo") || path.contains("local at"),
                    "Should resolve to Foo struct or local"
                );
            }
        }
    }
...
fn test() {
    let b = Bar { value: 10 };
        ...
        assert!(
            !result.is_empty(),
            "Should find goto target for Bar constructor"
        );

        for target in &result {
            if let Some(path) = target.pretty_path(&db) {
                assert!(
                    path.contains("Bar") || path.contains("local at"),
                    "Should resolve to Bar struct or local"
                );
            }
        }
    }
