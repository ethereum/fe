...
pub fn get_item_docstring(item: &ItemKind, hir_db: &dyn HirDb) -> Option<String> {
    let docstring = match item {
        ItemKind::Func(func) => func.attributes(hir_db).data(hir_db).clone(),
        ItemKind::Mod(mod_) => mod_.attributes(hir_db).data(hir_db).clone(),
        ItemKind::Struct(struct_) => struct_.attributes(hir_db).data(hir_db).clone(),
        ItemKind::Enum(enum_) => enum_.attributes(hir_db).data(hir_db).clone(),
        ItemKind::TypeAlias(type_alias) => type_alias.attributes(hir_db).data(hir_db).clone(),
        ItemKind::Trait(trait_) => trait_.attributes(hir_db).data(hir_db).clone(),
        ItemKind::Impl(impl_) => impl_.attributes(hir_db).data(hir_db).clone(),
        ...
        ItemKind::Contract(contract) => contract.attributes(hir_db).data(hir_db).clone(),
        _ => vec![],
    }
    .iter()
    .filter_map(|attr| {
