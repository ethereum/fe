...
fn main() {
    ...
    let b = 42;
        ...
        assert!(result.is_empty(), "Double colon :: should not trigger goto, but got {:?}", result);
        ...
        assert!(result2.is_empty(), "Equals sign = should not trigger goto, but got {:?}", result2);
        ...
        assert!(result4.is_empty(), "Whitespace should not trigger goto, but got {:?}", result4);
    }
...
fn main() {
    let b: Bar;
        ...
        let result = goto_definition_with_hir_synthesis(&db, top_mod, cursor, file).unwrap();
        assert!(!result.is_empty(), "Last character of identifier should trigger goto");
    }
}