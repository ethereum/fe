...
#[rstest(
    ...
    params,
    expected,
    case("bar", &[int_token(-10)], Some(int_array_token(&[-1]))),
)]
fn test_arrays(method: &str, params: &[ethabi::Token], expected: Option<ethabi::Token>) {
    with_executor(&|mut executor| {
        let harness =
            deploy_solidity_contract(&mut executor, "solidity/arrays.sol", "Foo", &[]);

        let exit = harness.capture_call(&mut executor, method, params);
        print!("{:?}", exit);
        //let exit = harness.test_function(&mut executor, method, params, expected.as_ref());
    })
}