...
use fe_compiler_test_utils::revm::{uint_token, string_token, bool_token, uint_token_from_dec_str, address_token};
...
#[test]
fn uniswap_contracts_revm() {
    let mut fevm = Fevm::default();
    let alice = fevm.create_account_with_balance(0_u64);
    ...
    let factory_harness = deploy_contract(
        ...
        "demos/uniswap.fe",
        "UniswapV2Factory",
        &[address_token(H160::default())],
    );

    let factory_address = address_token(factory_harness.address);
    ...
    let (pair_contract_address, account) = fevm.get_account_by_address(&pair_address.clone().into_address().expect("not an address")).unwrap();
    let pair_harness = fevm.load_contract_from_file(pair_contract_address.clone(), "demos/uniswap.fe", "UniswapV2Pair");
    pair_harness.test_function(&mut fevm, "factory", &[], Some(&factory_address));


    // Check that the token0 address is set correctly in the pair contract
    pair_harness.test_function(&mut fevm, "token0", &[], Some(&token0_address));
