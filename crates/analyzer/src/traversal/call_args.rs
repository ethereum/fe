...
use crate::namespace::types::{
    Array, Base, Contract, FeString, FixedSize, FunctionParam, Integer, Struct, StructField, Tuple,
    Type, TypeDowncast, U256,
};
...
use crate::namespace::events::EventField;
...
impl LabeledParameter for (String, FixedSize) {
    fn label(&self) -> Option<&str> {
        Some(&self.0)
    }
    fn typ(&self) -> &FixedSize {
...
pub fn validate_arg_types<'a>(
    ...
    for (index, (param, arg)) in params.iter().zip(args.kind.iter()).enumerate() {
        let val_attrs = assignable_expr(scope, &arg.kind.value, Some(&param.typ().clone().into()))?;
        if param.typ() != &val_attrs.typ {
            ...
            scope.type_error(&msg, arg.kind.value.span, &param.typ(), &val_attrs.typ);
        }
    }
    Ok(())
}
