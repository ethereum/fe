...
impl FunctionSigId {
    ...
    pub fn direct_parent(&self, db: &dyn AnalyzerDb) -> Option<Item> {
        let data = self.data(db);
        data.parent
    }

    pub fn parent(&self, db: &dyn AnalyzerDb) -> Item {
        self.direct_parent(db)
            .unwrap_or(Item::Module(self.module(db)))
    }

    pub fn parent_is_module(&self, db: &dyn AnalyzerDb) -> bool {
