...
use hir::{
    hir_def::{
        scope_graph::ScopeId, Body, Expr, ExprId, ItemKind, Partial, Pat, PatId, PathId, Stmt,
        StmtId,
    },
...
pub fn resolve_goto<'db>(
    ...
    match result {
        ...
        LazyHirResult::ItemGenericParam(item, idx) => {
            vec![NavTarget::Scope(ScopeId::GenericParam(item, idx))]
        }
...
pub fn resolve_goto_full<'db>(
    ...
    if let Some(item) = find_enclosing_item(span_db, top_mod, cursor) {
        ...
        if let Some((seg_path, scope)) =
            find_path_segment_at_cursor(span_db, cursor, collector.paths)
        {
            match resolve_path_segment(db, seg_path, seg_path.segment_index(db), scope, true) {
                ...
                Err(err) => match err.kind {
                    PathResErrorKind::NotFound(bucket) => {
                        ...
                        if !scopes.is_empty() {
                            return scopes.into_iter().map(NavTarget::Scope).collect();
                        }
                    }
...
impl<'db, 'ast: 'db> hir::visitor::Visitor<'ast> for PathSpanCollector<'db> {
    fn visit_path(
        &mut self,
        ctxt: &mut hir::visitor::VisitorCtxt<'ast, hir::span::path::LazyPathSpan<'ast>>,
        path: PathId<'db>,
    ) {
