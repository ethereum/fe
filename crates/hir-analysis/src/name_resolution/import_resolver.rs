...
impl<'db> ImportResolver<'db> {
    pub(crate) fn new(db: &'db dyn HirAnalysisDb, ingot: IngotDescription<'db>) -> Self {
        Self {
            db,
            ingot,
...
impl<'db> IntermediateResolvedImports<'db> {
    fn new(ingot: IngotDescription<'db>) -> Self {
        Self {
...
impl NameRes<'_> {
    ...
    fn is_external(&self, db: &dyn HirAnalysisDb, ingot: IngotDescription) -> bool {
        match self.kind {
