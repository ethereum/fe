...
pub fn lower_monomorphized_func_signature(
    ...
    let mut_self = func.mut_self(db.upcast());
    let mut_ctx = func.mut_ctx(db.upcast());

    // mut_self same like not good name... It's good if somebody can recommend a better nam
    if mut_self.is_some() {
