...
* Statically built compiler versions for smoother installation process
* WASM compiler versions for enhanced portability and in-browser compilation of
  Vyper contracts

## Inherited goals

Rust Vyper aims to achieve most goals of the [existing Vyper
project](https://github.com/vyperlang/vyper).  Those goals are listed
[here](https://vyper.readthedocs.io/en/latest/index.html#principles-and-goals)
though we include the following summary of them here:
...
* Lack of code re-use features such:
  - Modifiers (re-usable assertions)
  - Method resolution and contract inheritance (although we're on the fence
    about this one)
  - Module imports
...
We wish to leverage the YUL IR format as a means for targeting both the eWASM
...
* Availability of modern idioms such as iterators, functional programming
  features, etc.
* Support from highly visible and established organizations such as Mozilla
...
facilitate improvements in the release process of Rust Vyper over Legacy Vyper.
New compiler versions could be offered as statically compiled binaries
(removing dependence on existing Python binaries) or as WASM binaries (opening
up the possibility to run Rust Vyper in-browser or, in the distant future,
on-chain).
...
methods to the Rust Vyper compiler and the code it generates.
